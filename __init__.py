...
def consultation_utilisateur():
    ...
    return render_template('consultation_utilisateur.html', data=data)

@app.route('/fiche_utilisateur/<int:id>')
def fiche_utilisateur(id):
    # Connexion à la base de données
    conn = sqlite3.connect('database2.db')
    cursor = conn.cursor()
    ...
    if utilisateur:
        return render_template('fiche_utilisateur.html', utilisateur=utilisateur)
    else:
...
@app.route('/rechercher_utilisateur', methods=['POST'])
